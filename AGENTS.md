...
- **Never run `git stash`, `git stash drop`, `git stash clear`, `git update-ref`, `git restore`, `git checkout -- <file>`, or any git plumbing command.** These are strictly off-limits — they silently capture or destroy the entire dirty worktree, including the user's unrelated edits. The only exception: the user may explicitly type the git stash command themselves in the terminal. If an agent-initiated stash or pop is unavoidable, ask the user for permission first, listing every file that would be affected.
- **If any git command produces conflict output, error output, or shows unexpected file counts, stop immediately.** Do not proceed, do not assume partial success, and do not ignore aborted operations. Report the exact output to the user and ask for guidance.
- **After every git command, check its output before taking the next step.** `git status --short` shows the full worktree state. If a stash apply/pop says `conflicts: N files`, those files need resolution — treating partial apply as success destroys data.
