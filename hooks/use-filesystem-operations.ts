...
export function useFilesystemOperations() {
  ...
  const showConflictDialog = useCallback((
    ...
    return new Promise((resolve) => {
      setPendingConflict({ type, sourcePath, targetPath, resolve });
    });
  }, []);
  ...
  const rename = useCallback(async (options: RenameOptions): Promise<FileOperationResult> => {
    ...
    try {
      ...
      if (conflict.exists && !overwrite) {
        ...
        const shouldOverwrite = await showConflictDialog('rename', oldPath, newPath);
        if (!shouldOverwrite) {
          return { success: false, error: 'Operation cancelled by user' };
        }
      }

      // Perform rename via API (read + write + delete)
      ...
      if (!writeResponse.ok) {
        throw new Error('Failed to write to new path');
      }
      ...
      if (!deleteResponse.ok) {
        throw new Error('Failed to delete old path');
      }
  ...
  const move = useCallback(async (options: MoveOptions): Promise<FileOperationResult> => {
    ...
    try {
      ...
      if (conflict.exists && !overwrite) {
        const shouldOverwrite = await showConflictDialog('move', sourcePath, targetPath);
        if (!shouldOverwrite) {
          return { success: false, error: 'Operation cancelled by user' };
        }
      }
      ...
      if (!writeResponse.ok) {
        throw new Error('Failed to write to target');
      }
