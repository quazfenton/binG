...
export const AGENT_ROLE_CONFIGS: Record<AgentRole, Omit<AgentRoleConfig, 'id'>> = {
  ...
  planner: {
    name: 'Planner',
    description: 'Breaks down complex tasks into structured step-by-step plans',
  ...
  architect: {
    name: 'Architect',
    description: 'Designs scalable system architectures and makes high-level design decisions',
  ...
  reviewer: {
    name: 'Reviewer',
    description: 'Provides constructive code reviews focusing on quality and best practices',
  ...
  researcher: {
    name: 'Researcher',
    description: 'Researches technical topics, gathers information, and synthesizes findings',
  ...
  refiner: {
    name: 'Refiner',
    description: 'Iterates on code to improve quality, performance, and maintainability',
...
export const ROLE_COMPATIBILITY: Record<AgentRole, AgentRole[]> = {
  ...
  simplifier: ['mentor', 'documenter', 'researcher', 'planner'],
...
export const VFS_FILE_EDITING_TOOL_PROMPT = [
  'CRITICAL: All file operations MUST use the provided filesystem tools via native function/tool calling.',
  'This means you CALL these tools through the function calling API — you do NOT output text that looks like',
  'JavaScript function calls, XML tags, heredoc blocks, or code-formatted file changes.',
  '',
  'AVAILABLE FILE TOOLS (call these via function calling, NOT as text output):',
  '• write_file(path, content, commitMessage?) — Create a new file or completely overwrite an existing file.',
  '  Correct: write_file(path="hello.py", content="print(\'Hello World\')")',
  '  Wrong: create_file(...), writeFile(...), write_file(file=...), write_file(code=...)',
  '• read_file(path) — Read the full content of a file. ALWAYS call this before editing an existing file.',
  '  Correct: read_file(path="src/App.tsx")',
  '  Wrong: readFile(...), read_file(file=...)',
  ...
  '  Correct: apply_diff(path="src/App.tsx", diff="--- a/src/App.tsx\\n+++ b/src/App.tsx\\n@@ ...")',
  '  Wrong: apply_diff(patch=...), apply_diff(changes=...)',
  '• delete_file(path, reason?) — Delete a file or directory.',
  '  Correct: delete_file(path="src/old.ts")',
  '  Wrong: deleteFile(...), remove_file(...)',
  '• list_files(path?, recursive?) — List files and directories in a folder.',
  '  Correct: list_files(path="src/")',
  '  Wrong: listFiles(...), ls(...)',
  '• search_files(query, path?, limit?) — Search across all files for a text pattern.',
  '  Correct: search_files(query="useState", path="src/")',
  '  Wrong: searchFiles(...), search_files(term=...)',
  ...
  '• Multiple files: batch_write([{path: "file1.ts", content: "..."}, {path: "file2.ts", content: "..."}])',
  '',
  'TERMINAL COMMANDS:',
