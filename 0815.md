...
## Overview
This document captures an actionable audit and plan for improvements across streaming behavior, mobile UX, controls, diff/edit consistency, autosuggest, provider defaults, and subfolder traversal. It also outlines a phased rollout and testing strategy. Each future iteration should append updates with a new date-stamped block, preserving prior context.

Goals:
- Reduce perceived latency and improve stability of token streaming
- Improve mobile UX with responsive, touch-friendly interactions
- Provide clear, discoverable controls with sensible defaults
- Ensure consistency and safety in code diff/edit operations
- Deliver helpful, low-noise autosuggestions
- Define robust provider defaults and fallbacks
- Document safe subfolder traversal practices
- Roll out changes safely with metrics and rollback
...
## Controls
1. Visibility and grouping
   - Group by: Model & Provider, Quality/Speed, Context/Memory, Safety, Experiments
2. Key controls
   - Provider selector (with default per workspace)
   - Model selector; temperature/top_p; max tokens; system prompt presets
   - Streaming toggle; autosuggest toggle with sensitivity level
   - Safety: max output length, block lists, redact PII toggle (where applicable)
   - Context options: attach files, include conversation history toggle
3. Discoverability
   - Collapsible advanced section; tooltips with concise explanations; reset-to-defaults
4. Persistence
   - Persist per workspace and per device; provide “use defaults” override
