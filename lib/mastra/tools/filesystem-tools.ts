...
/**
 * Get the current user ID for filesystem operations
 ...
 * Filesystem tools for LLM agent
 */
export const filesystemTools: FilesystemTool[] = [
  {
    name: 'read_file',
  ...
  
  {
    ...
    execute: async ({ path }: { path: string }): Promise<ToolCallResult> => {
      try {
        const userId = getCurrentUserId();
        const entries = await virtualFilesystem.listDirectory(userId, path);
        return {
          success: true,
  ...
  
  {
    ...
    execute: async ({ query, path }: { query: string; path?: string }): Promise<ToolCallResult> => {
      try {
        const userId = getCurrentUserId();
        const snapshot = await virtualFilesystem.getWorkspace(userId);
...
/**
 * Get a specific filesystem tool by name
 */
export function getFilesystemTool(name: string): FilesystemTool | undefined {
  return filesystemTools.find(t => t.name === name);
}
