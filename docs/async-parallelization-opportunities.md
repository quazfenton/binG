...
| **2** | `applyScript(target, script)` in `web/lib/orchestra/prompt-orchestrator/injection-planner.ts` (the existing `target: string` parameter is the agent history adapter surface) | Agent-history adapter: `target` is a free-form string; an agent's chat history is a string; `applyScript(chatHistory, script)` does the rest (idempotency-check + inject markers) | prompt-orchestrator owner | **✓ applied (in-scope-of-#1, 2026-07-08)** | **★★★★☆** | **Structural-safety**: in-scope of step 1s planner — no separate adapter layer needed; the `target: string` parameter is the adapter surface. The agent-history adapter is therefore a 1-line wrapper at the call site: `const updated = applyScript(agent.history, promptScript)`. **Blast radius**: zero — no code change to the foundation; the adapter is at the call site. **Note on the verdict-table ranking**: step 2 is NOT in the top-4 highest-ROI tier (1, 3, 7, 6) because it has no separate ROI — the value is captured by step 1s `applyScript` API. **First production caller (2026-07-08)**: `web/lib/orchestra/unified-agent-service.ts:L1512` (the auto-inject powers site in `processUnifiedAgentRequest`); applies `observeApplyScript(config.userMessage, PO_UNIFIED_AGENT_SCRIPT, 'unified-agent')` before the powers fan-out to BOTH the V1-API path (appendAutoInjectPowers → config.conversationHistory) AND non-history modes (buildAutoInjectUserMessage → autoInjectContext for OpenCodeEngine / StatefulAgent / Mastra). **Structural first-caller**: `PO_UNIFIED_AGENT_SCRIPT` (sourced from `web/lib/orchestra/prompt-orchestrator/default-scripts.ts` — was inline `PO_DEFAULT_SCRIPT` pre-consolidation) has `promptId: unified-agent-entry` and empty `steps: []`, so the inject path doesnt fire (only scan + idempotency run end-to-end). To unlock Tier 8 step 4 (round-trip writes) + step 8 (observability) on real production data, a follow-up apply must add a step to `PO_UNIFIED_AGENT_SCRIPT` in `default-scripts.ts` (or switch to `loadScript` for a disk-stored script). tsc clean for the target file; 24/24 vitest prompt-orchestrator foundation suite pass.  **Second production caller (2026-07-08, followup-d)**: `web/.bing-shared/services/scheduler/triggers/marker-scanner.ts` (the poll() loop with `observeApplyScriptFn(this.poDefaultScript, ...)` after the marker-firing for-loop) (marker-in-history trigger poll) — calls `this.observeApplyScriptFn(newContent, PO_MARKER_TAIL_SCRIPT, 'marker-tail')` AFTER the marker-firing for-loop completes; dynamic-imported in `start()` (parallel to scanMarkers, preserves boot-without-prompt-orchestrator contract); structural (empty `steps: []` makes the call a faithful pass-through). Wired via `observeApplyScriptFn?: (target, script, source) => string` injection option parallel to existing `scanMarkersFn` **Post-consolidation contract (2026-07-08)**: The bare `applyScriptFn?: (target, script) => string` was renamed to **`observeApplyScriptFn?: (target, script, source) => string`** once Tier 8 step 8 observability was wired — the wrapper adds the `source` arg as the Prometheus metric label. Separately, the previously inline `PO_DEFAULT_SCRIPT` const was promoted to **two per-call-site consts** at `web/lib/orchestra/prompt-orchestrator/default-scripts.ts`: `PO_UNIFIED_AGENT_SCRIPT` (promptId=`unified-agent-entry`, caller=`unified-agent-service.ts:L1512`) + `PO_MARKER_TAIL_SCRIPT` (promptId=`marker-tail-poll`, caller=`marker-scanner.ts`). `PO_MARKER_TAIL_SCRIPT` is now sourced from the shared `web/lib/orchestra/prompt-orchestrator/default-scripts.ts` module via dynamic-import in `start()` (preserves boot-without-prompt-orchestrator contract). **→ 2 production callers now exist; Tier 8 step 8 (observability) is eligible for de-defer (see row 8).** To go structural → behavioral (e.g. for step 4 round-trip writes or step 8 metrics), add a step to `PO_UNIFIED_AGENT_SCRIPT` (or `PO_MARKER_TAIL_SCRIPT` for the scheduler 2nd caller) in `default-scripts.ts` — or switch to `loadScript()` for a disk-stored script. |
...
| **8** | Observability/metrics — injection-history dashboard | n/a | n/a | **✓ applied (2026-07-08)** | **★★☆☆☆** | Additive consumer of step 1/3; no structural conflicts. See `docs/prompt-orchestrator-deferred-steps.md#step-8` + apply summary: observability.ts (Maps; sha-label dropped), metrics/route.ts (GET, text/plain; v0.0.4), observability.test.ts (5 cases). Wired at `unified-agent-service.ts:L1512` (source=`unified-agent`; was L1517 pre-consolidation, shifted after the `PO_DEFAULT_SCRIPT` → `PO_UNIFIED_AGENT_SCRIPT` consolidation into `default-scripts.ts`) + `marker-scanner.ts` (source=`marker-tail`; in `.bing-shared/*` — see [Monorepo Layout](MONOREPO_LAYOUT.md)). Operator-debug: `curl /api/orchestra/prompt-orchestrator/metrics`. Phase 2: steps 4/5/9 still deferred. |
