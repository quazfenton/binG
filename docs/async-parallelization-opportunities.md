...
| **8** | Observability/metrics — injection-history dashboard | n/a | n/a | **✓ applied (2026-07-08)** | **★★☆☆☆** | Additive consumer of step 1/3; no structural conflicts. See `docs/prompt-orchestrator-deferred-steps.md#step-8` + apply summary: observability.ts (Maps; sha-label dropped), metrics/route.ts (GET, text/plain; v0.0.4), observability.test.ts (5 cases). Wired at `unified-agent-service.ts:L1517` (source=`unified-agent`) + `marker-scanner.ts` (source=`marker-tail`; lives in the `.bing-shared/*` mirror — see [Monorepo Layout](MONOREPO_LAYOUT.md) for resolution priority). Operator-debug: `curl /api/orchestra/prompt-orchestrator/metrics`. Phase 2: steps 4/5/9 still deferred. |
