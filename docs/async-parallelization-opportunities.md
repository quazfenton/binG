...
| 53 | `enhanced-middleware.ts:533` (double `desktop-auth-bypass` module import) | two consecutive `await import()` for the SAME module | middleware / desktop-auth owner | ☐ pending APPLY | collapse to single destructured import + remove microtask hop, ~1-3ms/middleware-hit |
...
| 63 | `token-refresh.ts:259` (sequential `getUserConnections` for 11 providers) | sequential `for…of` over OAuth connection list | OAuth refresh team / Platform | ☐ pending APPLY | read-only graph fetch per isolated provider; no shared state mutation (≠ invalidated #64 which mutates `getOrRefreshUserTokens` backing store), ~30-150ms/token-refresh |
| 65 | `unified-agent-service.ts:1921` (sequential metric counters) | sequential `await this.metrics…` calls | observability / metrics owner | ☐ pending APPLY | decoupled telemetry writes, ~720B-2KB/req telemetry-overhead-recovery |
| 67 | `unified-agent-service.ts:2373` (sequential `injectContext` per history msg) | sequential `for…of` over chat history | unified-agent-service context team | ☐ pending APPLY | static context over static history, safe `Promise.all(map(...))`, ~5-25ms/req depending on history depth |
...
| 72 | **`lib/crewai/crew/events.ts:209`** (event listeners in `emit`) | sequential `await` over listener array inside `async emit` | CrewAI task loop owner | ☐ pending APPLY | **`Promise.allSettled`** (not `Promise.all`) so a single listener reject doesn't crash the CrewAI task loop. SHAPE difference from rest of apply list — listen carefully when applying |
...
9. **NEW-1 followup-d apply pass — 15 ☐ APPLY candidates from Group C verdict table** — order by structural-safety + dependency-graph to minimize reviewer-load and allow incremental rollback. **Phase-2 audit format with Apply site + Reviewer role columns is now the source of truth** (see Status Audit `### NEW-1 followup-d` section above). Apply order:
   ...
   6. **observability** (#65) — decoupled metric counters. Approx negligible wallclock but ~720B-2KB/req telemetry-overhead-recovery.
   ...
   8. **CREWAI listener fan-out** (#72) — MUST use **`Promise.allSettled`** (not `Promise.all`) — SHAPE difference from the rest of the apply list; failing to use allSettled will crash the CrewAI task loop on single-listener rejection. Listen carefully to reviewer on this one.
