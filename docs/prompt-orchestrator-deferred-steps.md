...
| **8** | `web/lib/orchestra/prompt-orchestrator/observability.ts` (new file) + Prometheus metrics endpoint at `web/app/api/orchestra/prompt-orchestrator/metrics/route.ts` | Observability/metrics — counters: `prompt_injection_total{source, promptId, mode}`, `prompt_idempotency_skip_total{promptId, step, sha}`, `prompt_apply_duration_seconds` histogram; gauges: `prompt_scripts_loaded`, `prompt_markers_in_history` | prompt-orchestrator owner + observability team | **✓ applied (2026-07-08)** | **★★☆☆☆** | **Structural-safety analysis**: this is the lowest-risk deferred step — it's a pure additive consumer of step 1's `scanMarkers` output + step 3's metadata.json + step 7's trigger event counts. No structural conflicts. **Applied 2026-07-08**: see observability.ts + metrics/route.ts + the apply-summary paragraph below. (Pre-apply defer rationale, kept for historical context: `metadata.json` grep + `scanMarkers(chatHistory)` + the inline `applyScript` return value. The ROI of a metrics dashboard scales with QPS, and the current QPS is well-served by ad-hoc grep) — **APPLIED 2026-07-08** (see apply-summary paragraph below). For full un-defer conditions see the apply summary.: (a) prompt injection is in the hot path of every chat turn (QPS > 10/s sustained); (b) operator debugging is bottlenecked on ad-hoc grep (e.g. an incident takes >30min to trace); OR (c) the team needs SLO-based alerting (e.g. "fire if `prompt_apply_duration_seconds{p99} > 100ms` for 5min"). |
