...
import { getOrCreateAnonymousSessionId, normalizeOwnerId } from '@/lib/utils/utils';
...
export function EnhancedDiffViewer({
  ...
  const {
    ...
    syncStatus
  } = useOPFS(normalizeOwnerId(sessionId || getOrCreateAnonymousSessionId()), { autoEnable: true });
...
export function OPFSSyncStatusBadge({ path }: { path: string }) {
  const { syncStatus, isEnabled } = useOPFS(normalizeOwnerId(getOrCreateAnonymousSessionId()), { autoEnable: true });
