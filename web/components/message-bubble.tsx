...
export default function MessageBubble({
  ...
  // Capture the selection text at the moment the context menu opens.
  // We must snapshot it here because any mousedown on the menu will clear
  // window.getSelection() before the menu item's onClick fires.
  ...
  // We listen on 'pointerup' rather than 'mousedown' so the menu item's
  // onClick fires before the menu is torn down.  Using 'mousedown' caused
  // the menu to close immediately — before any click event — making it
  // impossible to activate a menu item.
  useEffect(() => {
    if (!contextMenu) return
    const handlePointerUp = (event: PointerEvent) => {
      const target = event.target as Node | null
      // Only dismiss when the click landed outside the bubble (which
      // includes the context-menu div itself, since it's inside bubbleRef).
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} ${useCompactLayout ? 'mb-3' : 'mb-6'} group w-full px-1`}>
      <div
        ...
        style={{
          ...
          // Enable touch scrolling for long content on mobile; on desktop use
          // 'clip' to stop paint-overflow without creating a scroll container —
          // 'visible' caused half-bubble flicker during scroll repaints.
          overflowY: contentAnalysis.hasLongContent && layout.isMobile ? 'auto' : 'clip',
        ...
        onContextMenu={(event) => {
          // Snapshot the current text selection before the menu opens.
          // Any subsequent mousedown on a menu item would clear
          // window.getSelection(), so we must capture it here.
          contextMenuSelectionRef.current = window.getSelection?.()?.toString() ?? ''
          event.preventDefault()
          // Set menu position, clamped to viewport edges
          setContextMenu({
            x: Math.min(event.clientX, typeof window !== 'undefined' ? window.innerWidth - 200 : event.clientX),
            y: Math.min(event.clientY, typeof window !== 'undefined' ? window.innerHeight - 120 : event.clientY),
          })
        }}
        onTouchStart={() => {
          // Do NOT preventDefault on touch — native scroll, selection, and gestures must work.
          // Just clear any stale context menu so it doesn't linger after a tap.
      ...
      >
        ...
        {/* Custom right-click context menu. Native menu is suppressed so we can offer
            a copy entry that works on the full message and a "select all" hint. */}
        {contextMenu && (
          <div
