...
import { getGitHubToken, githubApi } from '@/lib/integrations/github/github-oauth';
    ...
    for (const file of blobFiles.slice(0, 50)) { // Limit to 50 files for performance
      try {
        const fileResponse = await githubApi<any>(
          `/repos/${targetOwner}/${targetRepo}/contents/${file.path}?ref=${encodeURIComponent(targetBranch)}`,
          token
        );
