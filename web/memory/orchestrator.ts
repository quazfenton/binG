...
export class Orchestrator {
  ...
  static async fromPath(
    path: string,
    config: Omit<OrchestratorConfig, "projectId">
  ): Promise<Orchestrator> {
    const projectId = await projectIdFromPath(path);
    return new Orchestrator({ ...config, projectId });
  }
  ...
  /** Notify orchestrator which files are open in current tab */
