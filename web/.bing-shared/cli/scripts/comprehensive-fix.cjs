...
// 5. Regex patterns - both patterns need double backslash
console.log('5. Regex patterns...');
let f1 = 0, f2 = 0;
// Fix /\//g -> /\\//g (first pattern)
const bad1 = sBS + '/g';
const good1 = dBS + '/g';
// Fix \/+$/ -> \\/+$/ (second pattern)
const bad2 = sBS + '/+$/';
...
// 6. DO NOT touch duplicate login - leave it as is since we don't know which to remove
console.log('6. Login commands - leaving as is (pre-existing)');

// Write
fs.writeFileSync(binPath, lines.join('\n'), 'utf-8');
console.log('\n=== Done ===');
console.log('Changes made:', changes.length > 0 ? changes.join(', ') : 'none');
...
console.log('- Broken return fixed:', !v.some((l, i) => l.trim() === 'return  +' && v[i+1]?.trim() === ';') ? '✓' : '✗');

// Check regex patterns
const regexLines = v.filter(l => l.includes('replace(/'));
console.log('- Replace calls:', regexLines.length);