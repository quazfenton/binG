...
async function initializeSessionNaming(): Promise<void> {
  ...
  try {
    ...
    while (attempts < 3) {
      const response = await fetch(`${getBaseUrl()}/api/filesystem/list?path=${encodeURIComponent('workspace/sessions')}`);
    ...
    for (const node of nodes) {
      ...
      if (match) {
        ...
        // Register non-sequential names too (including suffixed versions like 000a, 001a, alpha-2, my-workspace, etc.)
        usedNames.add(name.toLowerCase());
      }
    }
 ...
 * @param isNewProject - Whether this is a new workspace (first generation)
...
export async function generateSessionName(
  ...
  try {
    ...
    // Rule 1: If it's a new workspace, has only 1 folder, and LLM suggested a name,
...
export async function sessionNameExists(name: string): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${getBaseUrl()}/api/filesystem/list?path=${encodeURIComponent(`workspace/sessions/${name}`)}`);
...
export function checkFileConflicts(
  ...
  if (!isExistingSession) {
    // New workspace - no conflicts possible
