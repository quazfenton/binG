 ...
 *
 * This prevents unnecessary nesting like workspace/sessions/001/my-app/...
 * when the LLM created a single workspace folder.
 *
...
export async function hoistSessionFolder(
  ...
  try {
    const basePath = `workspace/sessions/${sessionId}`;
    ...
    const newBasePath = `workspace/sessions/${folderName}`;
