...
const mustStayNode = new Set([
  'auth', 'agent', 'sandbox', 'filesystem', 'storage', 'terminal', 
  'docker', 'user', 'backend', 'livekit', 'mastra', 'orchestration', 
  'kernel', 'providers', 'code', 'integrations', 'blaxel', 'smithery',
  'automations', 'antigravity', 'preview', 'devbox', 'modal', 'runloop',
  'codesandbox'
]);
...
function addEdgeToRoute(filePath) {
  ...
  if (content.includes("runtime = 'nodejs'")) return false;

  const relativePath = path.relative(apiDir, filePath);
  const parts = relativePath.split(path.sep);
  const category = parts[0];

  if (mustStayNode.has(category)) return false;
